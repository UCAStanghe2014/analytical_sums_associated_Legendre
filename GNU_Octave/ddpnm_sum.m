...
w = sqrt(1+t.^2-2.*t.*c);
s = sqrt(1-c.^2);
p1 =  9.*s.^2.*t.*(1+15.*c.*t+18.*((-2)+c.^2).*t.^2+c.*((-39)+c.^2).*t.^3+(-3).*((-20)+c.^2).*t.^4+(-9).*c.*t.^5+(-8).*t.^6).*w.^(-11)+((-4).*c.^4.*t.^3+9.*c.^3.*t.^2.*((-6)+t.^2)+(-3).*t.*((-5)+13.*t.^2+3.*t.^4)+3.*c.^2.*t.*((-10)+27.*t.^2+6.*t.^4)+c.*((-1)+72.*t.^2+(-66).*t.^4+8.*t.^6)).*w.^(-9);
p2 =  35.*s.^2.*t.^5.*w.^(-9)+(-1).*c.*w.^(-5)+(-5).*c.*t.^4.*w.^(-9).*(7.*s.^2+w.^2)+(-5).*t.^3.*w.^(-9).*((-7)+7.*c.^4+21.*s.^2+(-1).*w.^2+3.*s.^2.*w.^2)+t.*w.^(-7).*((-5)+15.*s.^2+(-2).*w.^2+4.*s.^2.*w.^2)+c.*t.^2.*w.^(-9).*(35.*s.^2+5.*w.^2+25.*s.^2.*w.^2+w.^4);
p3 =  w.^(-7).*((-1).*c.*w.^2+c.*t.^2.*(5.*s.^2+2.*w.^2)+(-1).*t.*(w.^2+s.^2.*((-5)+10.*t.^2+(-2).*w.^2)));
p4 =  w.^(-5).*(3.*s.^2.*t+(-1).*c.*w.^2);
